...
import { getNewsPostsCached } from "@/lib/data/cached-queries";
...
export const metadata = {
  title: "News - VehicleClaim AI",
  description:
    "Stay up to date with the latest news from VehicleClaim AI.",
};

// TODO: Remove example posts once real data exists in Appwrite
const EXAMPLE_POSTS: NewsPostDocument[] = [
  {
    $id: "example-1",
    $collectionId: "",
    $databaseId: "",
...
export default async function NewsPage() {
  ...
  const dbPosts = await getNewsPostsCached();
  // Use example posts as fallback when no real posts exist
  const posts = dbPosts.length > 0 ? dbPosts : EXAMPLE_POSTS;
